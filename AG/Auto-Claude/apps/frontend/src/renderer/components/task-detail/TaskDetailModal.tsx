...
import { startTask, stopTask, submitReview, recoverStuckTask, deleteTask, persistTaskStatus, useTaskStore } from '../../stores/task-store';
...
function TaskDetailModalContent({ open, task, onOpenChange, onSwitchToTerminals, onOpenInbuiltTerminal }: { open: boolean; task: Task; onOpenChange: (open: boolean) => void; onSwitchToTerminals?: () => void; onOpenInbuiltTerminal?: (id: string, cwd: string) => void }) {
  ...
  const handleStartStop = async () => {
    if (state.isRunning && !state.isStuck) {
      ...
      if (state.isIncomplete) {
        ...
        if (!isValid) {
          toast({
            title: t('tasks:detail.cannotResume'),
            description: t('tasks:detail.cannotResumeDescription'),
            variant: 'destructive',
            duration: 5000,
          });
          return;
  ...
  const handleDismissIncomplete = async () => {
    const result = await persistTaskStatus(task.id, 'backlog');
    if (!result.success) {
      toast({
  ...
  const renderPrimaryAction = () => {
    // Ephemeral tasks (AutoGen/Pipeline) can't be started/stopped from Auto-Claude
        ...
        <DialogPrimitive.Portal>
          ...
          >
            ...
            <div className="p-5 pb-4 border-b border-border shrink-0">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-1 shrink-0 electron-no-drag">
                  <Button
                    ...
                    disabled={state.isEphemeral || (state.isRunning && !state.isStuck)}
                  >
              ...
              {/* Warnings - compact inline (not applicable for ephemeral tasks) */}
              {!state.isEphemeral && (state.isStuck || state.isIncomplete) && (
                <div className="mt-3">
                  <TaskWarnings
                    ...
                    onDismiss={state.isIncomplete ? handleDismissIncomplete : undefined}
                  />
                </div>
              )}
            </div>
              ...
              <Tabs value={state.activeTab} onValueChange={state.setActiveTab} className="flex flex-col h-full">
                <TabsList className="w-full justify-start rounded-none border-b border-border bg-transparent px-5 h-auto shrink-0">
                  ...
                  <TabsTrigger
                    ...
                    className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none px-4 py-2.5 text-sm"
                  >
                    {t('tasks:detail.subtabs.subtasks')} ({task.subtasks.length})
                  </TabsTrigger>
                  <TabsTrigger
                    value="logs"
                    className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none px-4 py-2.5 text-sm"
                  >
                    {t('tasks:detail.subtabs.logs')}
                  </TabsTrigger>
                  {showFilesTab && (
                    <TabsTrigger
                      value="files"
                ...
                <TabsContent value="overview" className="flex-1 min-h-0 overflow-hidden mt-0">
                  <ScrollArea className="h-full">
                    <div className="p-5 space-y-5 overflow-x-hidden max-w-full">
                      ...
                      {/* Human Review Section - not applicable for ephemeral tasks */}
                      {state.needsReview && !state.isEphemeral && (
                        <>
                          <Separator />
                          <TaskReview
                            task={task}
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="text-sm text-muted-foreground space-y-3">
                ...
                {task.id.startsWith('autogen_') || task.id.startsWith('pipeline_') ? (
                  <p className="text-muted-foreground">
                    This will remove the card from the Kanban board. The original data in AutoGen Studio is not affected.
                  </p>
                ) : (
