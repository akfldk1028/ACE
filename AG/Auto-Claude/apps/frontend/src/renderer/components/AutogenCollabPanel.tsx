...
export function AutogenCollabPanel({ isOpen, onClose, fullWidth = false }: AutogenCollabPanelProps) {
  ...
  const executeWorkflow = useCallback(async () => {
    ...
    try {
      // Get active project path so AG-ACE-BRIDGE targets the correct directory
  ...
  // Filter by active project to show only sessions bound to this project
  const fetchRuns = useCallback(async () => {
    try {
      // Get active project for filtering
      const { activeProjectId } = useProjectStore.getState();

      // ★ IPC를 통해 Main Process에서 8081 호출 (projectId로 필터링)
      const response = await window.electronAPI.getAutogenRunsDetailed(activeProjectId);
