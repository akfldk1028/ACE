...
import { GIT_BRANCH_REGEX } from './worktree-git-utils';
...
function atomicWriteFileSync(filePath: string, content: string): void {
  ...
  try {
    ...
    // On Windows, renameSync fails if target exists (unlike POSIX atomic rename)
...
export function registerTaskExecutionHandlers(
  ...
  ipcMain.on(
    ...
    async (_, taskId: string, _options?: TaskStartOptions) => {
      ...
      // Ephemeral tasks (AutoGen/Pipeline) cannot be started from Auto-Claude
      if (isEphemeralTaskId(taskId)) {
        console.warn(`[TASK_START] Ephemeral task - cannot start: ${taskId}`);
        mainWindow.webContents.send(IPC_CHANNELS.TASK_ERROR, taskId, 'Ephemeral tasks cannot be started');
        return;
      }
      ...
      // Validate specId format to prevent path traversal
      const SPEC_ID_REGEX = /^\d{3}-[a-z0-9][a-z0-9._-]*$/;
      if (!SPEC_ID_REGEX.test(task.specId)) {
        mainWindow.webContents.send(
          IPC_CHANNELS.TASK_ERROR,
          taskId,
          `Invalid spec ID format: ${task.specId}`
        );
        return;
      }
      ...
      // Validate branch names to prevent git command injection
      const rawBaseBranch = task.metadata?.baseBranch || project.settings?.mainBranch;
      const baseBranch = rawBaseBranch && GIT_BRANCH_REGEX.test(rawBaseBranch) ? rawBaseBranch : undefined;
      ...
      // CRITICAL: Persist status to implementation_plan.json BEFORE sending IPC response
      // to prevent status flip-flop. When getTasks() is called (on refresh), it reads
      // status from the plan file. Without persisting first, a quick refresh could read
      // the old status (e.g., 'human_review') and override the in-memory 'in_progress'.
      ...
      // Notify status change after file persistence
      mainWindow.webContents.send(
  ...
  ipcMain.on(IPC_CHANNELS.TASK_STOP, async (_, taskId: string) => {
    // Ephemeral tasks have no managed process
    if (isEphemeralTaskId(taskId)) {
      console.warn(`[TASK_STOP] Ephemeral task - no process to stop: ${taskId}`);
      return;
    }
    ...
    // Find task and project to persist status BEFORE sending IPC
    const { task, project } = findTaskAndProject(taskId);

    if (task && project) {
      // Persist status to implementation_plan.json BEFORE IPC to prevent flip-flop on refresh
  ...
  ipcMain.handle(
    ...
    async (
      ...
      // Ephemeral tasks have no review workflow
      if (isEphemeralTaskId(taskId)) {
        console.warn(`[TASK_REVIEW] Ephemeral task - review not applicable: ${taskId}`);
        return { success: true };
      }
  ...
  ipcMain.handle(
    ...
    async (
      ...
      // Ephemeral tasks have no disk state to persist
      if (isEphemeralTaskId(taskId)) {
        console.warn(`[TASK_UPDATE_STATUS] Ephemeral task status update (no-op on disk): ${taskId} → ${status}`);
        return { success: true };
      }
      ...
      try {
        ...
        if (status === 'in_progress' && !agentManager.isRunning(taskId)) {
          ...
          // Get base branch: task-level override takes precedence over project settings
          const rawBaseBranchForUpdate = task.metadata?.baseBranch || project.settings?.mainBranch;
          const baseBranchForUpdate = rawBaseBranchForUpdate && GIT_BRANCH_REGEX.test(rawBaseBranchForUpdate) ? rawBaseBranchForUpdate : undefined;
  ...
  ipcMain.handle(
    ...
    async (
      ...
      // Ephemeral tasks have no managed process to recover
      if (isEphemeralTaskId(taskId)) {
        console.warn(`[TASK_RECOVER_STUCK] Ephemeral task - recovery not applicable: ${taskId}`);
        return { success: true, data: { taskId, recovered: false, newStatus: 'backlog' as TaskStatus, message: 'Ephemeral task - no recovery needed' } };
      }
      ...
      try {
        ...
        if (autoRestart && project) {
          ...
          try {
            ...
            // Get base branch: task-level override takes precedence over project settings
            const rawBaseBranchForRecovery = task.metadata?.baseBranch || project.settings?.mainBranch;
            const baseBranchForRecovery = rawBaseBranchForRecovery && GIT_BRANCH_REGEX.test(rawBaseBranchForRecovery) ? rawBaseBranchForRecovery : undefined;
