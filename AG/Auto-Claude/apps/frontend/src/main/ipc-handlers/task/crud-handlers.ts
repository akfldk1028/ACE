...
export function registerTaskCRUDHandlers(agentManager: AgentManager): void {
          ...
          if (image.data) {
            try {
              ...
              if (!sanitizedFilename || sanitizedFilename === '.' || sanitizedFilename === '..') {
                console.error(`Invalid image filename: ${image.filename}`);
                continue;
              }

              // Decode base64 and save to file
              const buffer = Buffer.from(image.data, 'base64');
              const imagePath = path.resolve(attachmentsDir, sanitizedFilename);

              // Verify resolved path is within attachments directory
              if (!imagePath.startsWith(path.resolve(attachmentsDir) + path.sep)) {
                console.error(`Path traversal attempt blocked: ${image.filename}`);
                continue;
              }
  ...
  ipcMain.handle(
    ...
    async (_, taskId: string): Promise<IPCResult> => {
      // AutoGen ephemeral tasks (memory-only, no disk spec)
      if (isEphemeralTaskId(taskId)) {
        console.warn(`[TASK_DELETE] Ephemeral task removed: ${taskId}`);
        return { success: true };
      }
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        // AutoGen/Pipeline ephemeral tasks - no disk persistence
        if (isEphemeralTaskId(taskId)) {
          console.warn(`[TASK_UPDATE] Ephemeral task update (no-op on disk): ${taskId}`);
          return { success: true, data: undefined as unknown as Task };
        }
