"""
Orchestrator for AG/Auto-Claude
...
class Orchestrator:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        # Circuit breaker: track consecutive failures per adapter
        # After CIRCUIT_BREAKER_THRESHOLD consecutive failures, block for CIRCUIT_BREAKER_COOLDOWN_S
        self._circuit_breaker: Dict[str, Dict] = {}  # agent_type -> {failures, blocked_until}
        self.CIRCUIT_BREAKER_THRESHOLD = 5
        self.CIRCUIT_BREAKER_COOLDOWN_S = 30
            ...
            if task:
                ...
                try:
                    ...
                    try:
                        ...
                        # Prevent queue.fail() errors from crashing the main loop
                        self.logger.error(
                            "queue_fail_error",
                            task_id=task.id,
                            error=str(fail_err),
    ...
    async def _execute_pipeline(
        ...
        for i, stage in enumerate(pipeline.stages):
            ...
            # Accumulate context with size limit to prevent memory explosion
            if result.output:
                import sys
                output_size = sys.getsizeof(str(result.output))
                MAX_CONTEXT_SIZE = 5 * 1024 * 1024  # 5MB limit

                # Truncate large outputs to prevent unbounded growth
                if output_size > MAX_CONTEXT_SIZE:
                    truncated = str(result.output)[:MAX_CONTEXT_SIZE]
                    context[f"stage_{i}"] = {"_truncated": True, "summary": truncated[:1000]}
                    self.logger.warning("stage_output_truncated", stage=i, size=output_size)
                else:
    ...
    def _check_circuit_breaker(self, agent_key: str) -> Optional[str]:
        """Check if circuit breaker is open for an agent. Returns error message if blocked."""
        cb = self._circuit_breaker.get(agent_key)
        if not cb:
            return None
        if cb.get("blocked_until"):
            if datetime.now() < cb["blocked_until"]:
                remaining = (cb["blocked_until"] - datetime.now()).seconds
                return f"Circuit breaker open for {agent_key}: {remaining}s remaining"
            else:
                # Cooldown expired, reset
                self._circuit_breaker[agent_key] = {"failures": 0, "blocked_until": None}
        return None

    def _record_circuit_breaker(self, agent_key: str, success: bool) -> None:
        """Record success/failure for circuit breaker tracking."""
        if agent_key not in self._circuit_breaker:
            self._circuit_breaker[agent_key] = {"failures": 0, "blocked_until": None}
        cb = self._circuit_breaker[agent_key]
        if success:
            cb["failures"] = 0
            cb["blocked_until"] = None
        else:
            cb["failures"] += 1
            if cb["failures"] >= self.CIRCUIT_BREAKER_THRESHOLD:
                from datetime import timedelta
                cb["blocked_until"] = datetime.now() + timedelta(seconds=self.CIRCUIT_BREAKER_COOLDOWN_S)
                self.logger.warning("circuit_breaker_open", agent=agent_key, cooldown=self.CIRCUIT_BREAKER_COOLDOWN_S)
    ...
    async def _execute_sequential_stage(
        ...
        cb_error = self._check_circuit_breaker(agent_type.value)
        if cb_error:
            return Result(
                task_id=task.id,
                status=ResultStatus.FAILED,
        ...
        try:
            ...
            if result.status == ResultStatus.SUCCESS:
                ...
                self._record_circuit_breaker(agent_type.value, True)
            else:
                self.registry.record_failure(agent_type)
                self._record_circuit_breaker(agent_type.value, False)
    ...
    async def _initialize_shared_memory(self) -> None:
        ...
        try:
            self._shared_memory = SharedMemoryClient(
                ...
                source_name="ag-auto-claude-orchestrator",
            )
    ...
    def _setup_signal_handlers(self) -> None:
        ...
        try:
            ...
            # Windows doesn't support add_signal_handler in asyncio loop
            # Fall back to signal.signal() for graceful shutdown on Windows
            if sys.platform == 'win32':
                def _win_handler(signum, frame):
                    self._stop_event.set()
                    self.logger.info("signal_received_windows", signal=signum)
