...
// When projectId is provided, tasks are bound to that project instead of 'autogen'
function autogenRunToTasks(run: {
  ...
  // Use provided projectId if bound, otherwise fall back to 'autogen'
  const taskProjectId = projectId || 'autogen';
  tasks.push({
...
export function KanbanBoard({ tasks, onTaskClick, onNewTaskClick, onRefresh, isRefreshing }: KanbanBoardProps) {
  ...
  // Get current active project for AutoGen session filtering
  ...
  // Use ref for toast to avoid recreating processQueue when toast changes
  ...
  const processQueue = useCallback(async () => {
    ...
    while (triggerQueueRef.current.length > 0) {
      ...
      try {
        ...
        // Look up current project path from store so AG-ACE-BRIDGE targets the right directory
        ...
        if (response.success && response.data?.success) {
          ...
          // ★ Auto-bind this session to the current project
          if (currentProjectId && currentProject?.path) {
            try {
              await window.electronAPI.autogenBindSession(
                currentProjectId,
                currentProject.path,
                parseInt(sessionId, 10)
              );
              console.log(`[24/7] Auto-bound session ${sessionId} to project ${currentProjectId}`);
            } catch (bindErr) {
              console.debug('[24/7] Auto-bind failed (Bridge may be down):', bindErr);
            }
          }
        } else {
  ...
  }, []); // Stable: no deps since toast is accessed via ref
  ...
  useEffect(() => {
    const MAX_TRIGGER_LOG_ENTRIES = 100;
    const MAX_PROCESSED_SESSIONS = 200;

    const cleanup = setInterval(() => {
      // Clean triggerLogRef
      const entries = Object.entries(triggerLogRef.current);
      if (entries.length > MAX_TRIGGER_LOG_ENTRIES) {
        const removable = entries.filter(([, v]) => v.status === 'done' || v.status === 'error');
        const toRemove = removable.slice(0, entries.length - MAX_TRIGGER_LOG_ENTRIES);
      ...
      // Clean processedSessionsRef to prevent unbounded growth
      if (processedSessionsRef.current.size > MAX_PROCESSED_SESSIONS) {
        const arr = Array.from(processedSessionsRef.current);
        const toKeep = arr.slice(arr.length - MAX_PROCESSED_SESSIONS);
        processedSessionsRef.current = new Set(toKeep);
      }
    }, 60 * 60 * 1000); // Every hour
    return () => clearInterval(cleanup);
  }, []);

  // ★ Listen for ephemeral task deletion (AutoGen / Pipeline cards)
  useEffect(() => {
    const handler = (e: Event) => {
      const { taskId } = (e as CustomEvent).detail;
      setAutogenTasks(prev => prev.filter(t => t.id !== taskId));
      setPipelineTasks(prev => prev.filter(t => t.id !== taskId));
    };
    window.addEventListener('autogen-task-deleted', handler);
    return () => window.removeEventListener('autogen-task-deleted', handler);
  }, []);
  ...
  useEffect(() => {
    const fetchAutogenResults = async () => {
      try {
        // Pass activeProjectId to filter sessions bound to this project via Bridge
        const detailedResponse = await window.electronAPI.getAutogenRunsDetailed(activeProjectId);
        if (detailedResponse.success && detailedResponse.data && detailedResponse.data.length > 0) {
          ...
          for (const run of detailedResponse.data) {
            ...
            // Pass activeProjectId so tasks are bound to the current project
            const sessionTasks = autogenRunToTasks(run, activeProjectId);
      ...
      try {
        ...
        if (detailedResponse.success && detailedResponse.data) {
          ...
          for (const run of detailedResponse.data) {
            ...
            // Pass activeProjectId so tasks are bound to the current project
            detailedTasks.push(...autogenRunToTasks(run, activeProjectId));
          }
    ...
    // Await initialFetch before starting poll to prevent race condition
    // where the 3-second poll fires before initial load completes
    let cancelled = false;
    initialFetch().then(() => {
      if (!cancelled) {
  ...
  }, [processQueue, activeProjectId]); // ★ Re-fetch when project changes
  ...
  const handleStatusChange = async (taskId: string, newStatus: TaskStatus, providedTask?: Task) => {
    ...
    // AutoGen cards are ephemeral (no disk spec) - update local state only
    // Check by task ID prefix rather than projectId, since AutoGen tasks may now be bound to real projects
    if (taskId.startsWith('autogen_') || taskId.startsWith('pipeline_')) {
      useTaskStore.getState().updateTaskStatus(taskId, newStatus);
      return;
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-xl">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-semibold">{t('tasks:pipeline.createTitle')}</h3>
            </div>
            <p className="text-sm text-muted-foreground mb-4">
              {t('tasks:pipeline.createDescription')}
            </p>

            <div className="space-y-3">
              <div>
                <label className="block text-sm font-medium mb-1">{t('tasks:pipeline.projectPath')}</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium mb-1">{t('tasks:pipeline.projectName')}</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium mb-1">{t('tasks:pipeline.description')}</label>
                <textarea
            ...
            <div className="flex justify-end gap-2 mt-5">
              ...
              >
                ...
                {isCreatingProject ? t('tasks:pipeline.creating') : t('tasks:pipeline.createAndPlan')}
              </Button>
            </div>
          </div>
        </div>
