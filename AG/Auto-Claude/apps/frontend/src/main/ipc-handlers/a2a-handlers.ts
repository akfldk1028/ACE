    ...
    async (_event, projectId?: string): Promise<IPCResult<{
      ...
      try {
        ...
        if (sessionsResponse.ok) {
          autogenReachable = true; // ★ 8081 연결 성공 플래그
      ...
      // ★ 8081 연결됐지만 complete 데이터 없음 → success: true, data: null
      if (autogenReachable) {
        debugLog('AutoGen Studio reachable but no complete data');
        return { success: true, data: null };
      }

      // ★ 2차 폴백: SharedMemory(8101) 조회 (8081 unreachable 시만)
      const url = getSharedMemoryUrl(projectId);
 ...
 *
 * When projectId is provided, queries AG-ACE-BRIDGE for bound sessions
 * and filters to only return those. Falls back to all sessions if Bridge is down.
 */
export function registerGetAutogenRunsDetailed(): void {
  ipcMain.handle(
    IPC_CHANNELS.AUTOGEN_GET_RUNS_DETAILED,
    async (_event, projectId?: string): Promise<IPCResult<Array<{
      sessionId: number;
      runId: number;
      status: string;
      task: string;
      ...
      debugLog('getAutogenRunsDetailed called', { projectId });
      ...
      try {
        ...
        // ★ Project filtering via AG-ACE-BRIDGE bindings
        // When projectId is provided, only return sessions bound to that project
        let boundSessionIds: Set<number> | null = null;
        if (projectId) {
          try {
            const bindingsRes = await fetch(
              `${AG_ACE_BRIDGE_URL}/bridge/bindings/project/${projectId}/sessions`,
              { headers: { Accept: 'application/json' } }
            );
            if (bindingsRes.ok) {
              const bindingsData = await bindingsRes.json();
              boundSessionIds = new Set(bindingsData.session_ids || []);
              debugLog(`Filtering by project ${projectId}: ${boundSessionIds.size} bound sessions`);
            }
          } catch (bindErr) {
            // Bridge not available - fall back to showing all sessions
            debugLog('Bridge not available for binding filter, showing all sessions', bindErr);
          }
        }

        // Filter sessions if we have binding info
        if (boundSessionIds !== null) {
          sessions = sessions.filter((s: { id: number }) => boundSessionIds!.has(s.id));
          debugLog(`Filtered to ${sessions.length} sessions for project ${projectId}`);
        }
...
// ============================================================================
// AutoGen Session Binding IPC Handlers (★ Project Binding - 2026-02)
// ============================================================================

const AG_ACE_BRIDGE_URL = 'http://localhost:8080';

/**
 * Bind an AutoGen session to a project via AG-ACE-BRIDGE
 */
export function registerAutogenBindSession(): void {
  ipcMain.handle(
    IPC_CHANNELS.AUTOGEN_BIND_SESSION,
    async (
      _event,
      projectId: string,
      projectPath: string,
      ...
      debugLog('autogenBindSession called', { projectId, projectPath, sessionId, teamId });

      try {
        const response = await fetch(`${AG_ACE_BRIDGE_URL}/bridge/bindings/bind`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        const msg = error instanceof Error ? error.message : 'Failed to bind session';
        debugLog('autogenBindSession failed:', msg);
...
/**
 * Unbind an AutoGen session from a project
 */
export function registerAutogenUnbindSession(): void {
  ipcMain.handle(
    IPC_CHANNELS.AUTOGEN_UNBIND_SESSION,
    async (
      _event,
      projectId: string,
      sessionId: number
    ): Promise<IPCResult<{ success: boolean; message: string }>> => {
      debugLog('autogenUnbindSession called', { projectId, sessionId });

      try {
        const response = await fetch(`${AG_ACE_BRIDGE_URL}/bridge/bindings/unbind`, {
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            project_id: projectId,
        ...
        const msg = error instanceof Error ? error.message : 'Failed to unbind session';
        debugLog('autogenUnbindSession failed:', msg);
        return { success: false, error: msg };
      }
    }
  );
...
/**
 * Get AutoGen sessions not bound to any project
 */
export function registerAutogenGetUnboundSessions(): void {
  ipcMain.handle(
    IPC_CHANNELS.AUTOGEN_GET_UNBOUND_SESSIONS,
    async (
      _event,
      allSessionIds: number[]
    ): Promise<IPCResult<{ unbound_session_ids: number[]; count: number }>> => {
      debugLog('autogenGetUnboundSessions called', { count: allSessionIds.length });

      try {
        const sessionIdsStr = allSessionIds.join(',');
        const response = await fetch(
          `${AG_ACE_BRIDGE_URL}/bridge/bindings/unbound?session_ids=${sessionIdsStr}`,
          { headers: { Accept: 'application/json' } }
        );
        ...
        return {
          success: true,
          data: {
            unbound_session_ids: result.unbound_session_ids || [],
            count: result.count || 0
          }
        };
      } catch (error) {
        const msg = error instanceof Error ? error.message : 'Failed to get unbound sessions';
        debugLog('autogenGetUnboundSessions failed:', msg);
        // Return all as unbound if Bridge is down
        return {
          success: true,
          data: { unbound_session_ids: allSessionIds, count: allSessionIds.length }
        };
      }
    }
  );
...
export function registerWorkflowExecute(): void {
  ipcMain.handle(
    ...
    async (
      ...
      debugLog('workflowExecute called', { task, complexity, autoMerge, projectPath });
...
export function registerA2AHandlers(): void {
  ...
  // AutoGen Session Binding handlers (★ Project Binding)
  registerAutogenBindSession();
  registerAutogenUnbindSession();
  registerAutogenGetUnboundSessions();
