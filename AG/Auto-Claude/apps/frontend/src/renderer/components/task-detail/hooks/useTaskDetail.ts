...
import { checkTaskRunning, isIncompleteHumanReview, isEphemeralTask, getTaskProgress, useTaskStore, loadTasks } from '../../../stores/task-store';
...
export function useTaskDetail({ task }: UseTaskDetailOptions) {
  ...
  const isEphemeral = isEphemeralTask(task);
  ...
  useEffect(() => {
    ...
    // Ephemeral tasks (AutoGen/Pipeline) have no managed process - skip stuck detection
    if (isEphemeral) {
      setIsStuck(false);
    ...
    // 'planning' phase: Reset hasCheckedRunning so stuck detection
    // re-triggers when task transitions to 'coding'
  ...
  // Ephemeral tasks have no worktrees - skip
  useEffect(() => {
    if (needsReview && !isEphemeral) {
  ...
  // Ephemeral tasks have no spec directory on disk - skip
  useEffect(() => {
    if (!selectedProject || isEphemeral) return;
