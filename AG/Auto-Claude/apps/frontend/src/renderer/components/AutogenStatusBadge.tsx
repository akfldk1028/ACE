...
export function AutogenStatusBadge({ className }: AutogenStatusBadgeProps) {
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      // Electron 모드: IPC로 8081 체크 (Main Process가 직접 호출, CORS 없음)
      if (window.electronAPI?.getAutogenLatest) {
