...
class TaskQueue:
    ...
    @contextmanager
    def _get_connection(self, immediate: bool = False):
        """Get database connection with context manager.

        Args:
            immediate: If True, use BEGIN IMMEDIATE to acquire write lock
                      immediately, preventing race conditions in read-modify-write
                      operations like dequeue.
        """
    ...
    def dequeue(self) -> Optional[Task]:
        ...
        """
        # Use BEGIN IMMEDIATE to prevent race condition where two concurrent
        # dequeue calls could SELECT the same pending task before either UPDATEs it
        with self._get_connection(immediate=True) as conn:
    ...
    MAX_JSON_FIELD_SIZE = 1024 * 1024  # 1MB limit per JSON field

    def _safe_json_loads(self, data: str, field_name: str, default=None):
        """Parse JSON with size validation and error handling."""
        if not data:
            return default if default is not None else {}
        if len(data) > self.MAX_JSON_FIELD_SIZE:
            self.logger.warning("json_field_too_large", field=field_name, size=len(data))
            return default if default is not None else {}
        try:
            return json.loads(data)
        except (json.JSONDecodeError, ValueError) as e:
            self.logger.warning("json_parse_error", field=field_name, error=str(e))
            return default if default is not None else {}
    ...
    def _row_to_task(self, row: sqlite3.Row) -> Task:
        ...
        return Task(
            ...
            input=self._safe_json_loads(row["input"], "input"),
            context=self._safe_json_loads(row["context"], "context"),
            requirements=self._safe_json_loads(row["requirements"], "requirements", []),
            metadata=self._safe_json_loads(row["metadata"], "metadata"),
