# law/ - 법규 검색 AI 채팅 UI (Legacy)

> **Note (2026-02-23)**: 이 UI는 ARR Legacy Frontend. 현재 주력 프론트엔드는 `AG-frontend/`.
> Backend endpoints가 변경됨: `/agents/law/api/*` → `/law/*` (Django law proxy → :8011).
...
| 백엔드 | 역할 |
|--------|------|
| `law/views.py` | 검색 프록시 (→ :8011) + SearchLog |
| `law/urls.py` | `/law/*` 라우팅 |
| `AG/agent/law-domain-agents/server.py` | 실제 검색 엔진 (FastAPI, :8011) |

> **Note**: 기존 `/agents/law/api/*` 경로는 폐기됨. 현재 `/law/*` 사용.
