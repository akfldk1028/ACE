...
export function createA2AAPI(): A2AAPI {
  return {
    ...
    // When projectId is provided, only returns sessions bound to that project
    getAutogenRunsDetailed: (projectId?: string): Promise<IPCResult<Array<{
      sessionId: number;
      runId: number;
      status: string;
      task: string;
      ...
      invokeIpc(IPC_CHANNELS.AUTOGEN_GET_RUNS_DETAILED, projectId),
    ...
    autogenBindSession: (
      ...
      invokeIpc(IPC_CHANNELS.AUTOGEN_BIND_SESSION, projectId, projectPath, sessionId, teamId),

    // Unbind an AutoGen session from a project
    autogenUnbindSession: (
      projectId: string,
      sessionId: number
    ): Promise<IPCResult<{ success: boolean; message: string }>> =>
      invokeIpc(IPC_CHANNELS.AUTOGEN_UNBIND_SESSION, projectId, sessionId),

    // Get AutoGen sessions not bound to any project
    autogenGetUnboundSessions: (
      allSessionIds: number[]
    ): Promise<IPCResult<{ unbound_session_ids: number[]; count: number }>> =>
      invokeIpc(IPC_CHANNELS.AUTOGEN_GET_UNBOUND_SESSIONS, allSessionIds),
    ...
    workflowExecute: (
      ...
      invokeIpc(IPC_CHANNELS.WORKFLOW_EXECUTE, task, complexity, autoMerge, projectPath),
