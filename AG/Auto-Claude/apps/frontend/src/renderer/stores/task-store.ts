 ...
 * Prioritizes exact id match to prevent collisions where one task's specId
 * could match another task's id.
 * Returns -1 if not found.
 */
function findTaskIndex(tasks: Task[], taskId: string): number {
  // First try exact id match
  const idIndex = tasks.findIndex((t) => t.id === taskId);
  if (idIndex !== -1) return idIndex;
  // Fall back to specId match
  return tasks.findIndex((t) => t.specId === taskId);
}
...
export async function deleteTask(
  ...
  try {
    ...
    if (result.success) {
      // Remove from local state (native tasks)
      store.setTasks(store.tasks.filter(t => t.id !== taskId && t.specId !== taskId));
      // Clear selection if this task was selected
      if (store.selectedTaskId === taskId) {
        store.selectTask(null);
      }
      // Notify KanbanBoard to remove AutoGen/pipeline ephemeral tasks from local state
      if (taskId.startsWith('autogen_') || taskId.startsWith('pipeline_')) {
        window.dispatchEvent(new CustomEvent('autogen-task-deleted', { detail: { taskId } }));
      }
      return { success: true };
    }
...
/**
 * Check if a task is ephemeral (AutoGen/Pipeline - memory-only, no disk spec).
 * Ephemeral tasks cannot be started/stopped/recovered and have no worktrees or logs on disk.
 */
export function isEphemeralTask(task: Task): boolean {
  return task.id.startsWith('autogen_') || task.id.startsWith('pipeline_');
}
...
export function isIncompleteHumanReview(task: Task): boolean {
  ...
  // AutoGen cards have no disk-backed subtasks; never flag as incomplete
  // Check by task ID prefix since AutoGen tasks may now be bound to real projects
  if (isEphemeralTask(task)) return false;
