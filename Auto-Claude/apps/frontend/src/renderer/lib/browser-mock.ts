...
const browserMockAPI: ElectronAPI = {
  ...
  workflowExecute: async (task: string, complexity: 'simple' | 'standard' | 'complex' = 'standard', autoMerge = false) => {
    console.log('[Browser Mock] workflowExecute:', { task, complexity, autoMerge });
    try {
      const response = await fetch('http://localhost:8080/workflow/execute', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ task, complexity, auto_merge: autoMerge }),
      });
      if (!response.ok) {
